...
public class KickClientCommandHandler extends AbstractCommand {
  ...
  private final String reasonForKickStartToKickedClient = "You've been kicked\n"
                                                          + "Reason: ";
  private final String getReasonForKickStartToAllOtherClients =
      " has been kicked\n"
      + "Reason: ";

  public KickClientCommandHandler() { opOnly = true; }
  ...
  private void kick(String[] args, int senderId) {
    if (senderId == server.getId()) {
      ...
      server.sendToClientWithText(server.getId(), senderId,
                                  MessageTypes.NORMAL_MESSAGE.getValue(),
                                  "Kicking client...");
    }
    ...
    if (args[0].equalsIgnoreCase("all")) {
      server.sendToAllClientsWithText(
          server.getId(), MessageTypes.KICK_CLIENT.getValue(),
          reasonForKickStartToKickedClient + reasonForKickMain);
    } else {
      ...
      if (getTargetId(multipleTargetsString)) {
        for (int i : targetList) {
          if (server.checkIfClientExists(i)) {
            ...
            server.sendToClientWithText(
                senderId, i, MessageTypes.KICK_CLIENT.getValue(),
                reasonForKickStartToKickedClient +
                    reasonForKickMain); // Kick the client
            server.sendToAllClientsWithText(
                senderId, MessageTypes.NORMAL_MESSAGE.getValue(),
                clientTargetUsername + getReasonForKickStartToAllOtherClients +
                    reasonForKickMain); // Tell all the other clients who got
                                        // kicked
          } else {
            if (senderId > 0) {
              server.sendToClientWithText(
                  server.getId(), senderId,
                  MessageTypes.NORMAL_MESSAGE.getValue(),
                  "User with ID " + i + " does not exist!");
            } else {
