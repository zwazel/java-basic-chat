...
public class Main {
  ...
  public Main() {
    ...
    while (!init())
      ; // While our input isn't valid
  }

  private boolean init() {
    System.out.print(
        "Do you want to host (0) or to join (1)? > "); // Tell the user what to
                                                       // do

    // TODO: catch error if not number
    switch (
        Integer.parseInt(scanner.nextLine())) { // get the input as string, and
    ...
    default:                                    // default, input is not valid
      System.out.println(
          "Invalid Input! Try again"); // tell the user whats wrong
