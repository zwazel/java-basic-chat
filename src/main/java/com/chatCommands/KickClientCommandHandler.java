...
import com.server.ServerClient;
...
public class KickClientCommandHandler extends AbstractCommand {
    ...
    private void kick(String[] args, int senderId) {
        if(senderId == server.getId()) {
            System.out.println("Kicking com.client...");
        } else {
            server.sendToClientWithText(server.getId(), senderId, MessageTypes.NORMAL_MESSAGE.getValue(), "Kicking com.client...");
        }
            ...
            if (getTargetId(multipleTargetsString)) {
                for (int i : targetList) {
                    if (server.checkIfClientExists(i)) {
                        ...
                        server.sendToClientWithText(senderId, i, MessageTypes.KICK_CLIENT.getValue(), reasonForKickStartToKickedClient + reasonForKickMain); // Kick the com.client
