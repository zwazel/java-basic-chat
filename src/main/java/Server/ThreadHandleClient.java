    ...
    while (running) { // While we're running
      try {
        DataInputStream dIn = new DataInputStream(
            s.getInputStream()); // Create a new dataInputStream
        ...
        case DISCONNECT:       // the client is disconnecting
          ...
          while (!removedClient) {
            if (server.removeClientFromMap(
                    myClientId)) { // get the ID of the client and disconnect
                                   // him)
        ...
        case NORMAL_MESSAGE: // Normal message
          server.sendToAllClientsWithText(
              myClientId, messageType.getValue(),
              dIn.readUTF()); // read text from the client and send it to the
                              // server and to all the other clients. and don't
                              // send it to me back
          break;
        ...
        case CLIENT_COMMAND:
          ...
          if (!server.handleCommandsClient(isOp, command, args, myClientId)) {
            server.sendToClientWithText(server.getId(), myClientId,
        ...
        default:
          ...
          System.out.println(
              "Got undefined Message Type from client in thread " + threadName +
              "! Message: " + message);
          server.sendToClientWithText(server.getId(), myClientId,
        ...
        System.out.println("Can't get message from client in thread " +
                           threadName);
      }
    }
  }
